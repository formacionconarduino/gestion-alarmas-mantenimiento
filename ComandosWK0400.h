...
//*******************definición del código de los comandos Winkhel
enum {
       ...
       CMD_APH_ELE,    //AÑADIR PROGRAMA HORARIO DE UN ELEMENTO
...
/*buscarElemento*****************************************************************************
*Busca en los HR de identificador de elemento a ver si está el elemento cuyo id_elemento 
*nos ha pasado el master WK0500 si lo encuentra devuelve la posición en la que lo ha encontrado
*si no lo encuentra devuelve un 0
*/
byte buscarElemento(int id_elemento)
{
byte p;  
for(p = HR_INFO ;  p <= HR_POTIN_ID ; p += 5) //la entrada analógica auxiliar la analizo por separado
   if(regs[p] == id_elemento)return p; // si lo encuentra devuelve posición
   if(regs[HR_AINAUX_ID] == id_elemento) return HR_AINAUX_ID;
   return 0;//NO lo ha encontrado
}  

byte posicion; //registro HR donde están los datos para ese elemento
//--------------------------------------------------
void ComandoError(){
...
posicion=buscarElemento(regs[1]);
//en regs[1] aunque no sé en que orden, están los 2 bytes de Id_elemento
switch(posicion){ //sólo puede ser un relé para poderse activar en el WK0400
  case  HR_RELE1_ID: HR_RELE2_ID: HR_RELE3_ID: HR_RELE4_ID:  
      regs[posicion + OFFSET_VESC]|=1;//ponemos un 1 en la escena cero(dejando los otros valores de escena) que es la que se activará al final
      break;
      }
}
//--------------------------------------------------
...
switch(posicion){ //sólo puede ser un relé para poderse activar en el WK0400
  case  HR_RELE1_ID: HR_RELE2_ID: HR_RELE3_ID: HR_RELE4_ID:  
      regs[posicion + OFFSET_VESC]&=0xFFFE;//ponemos un 0 en la escena cero(dejando los otros valores de escena) que es la que se activará al final
      break;
      }
}
//--------------------------------------------------
...
//Pregunta: en los 16 bits del argumento sólo puede haber un bit a 1
}
//--------------------------------------------------
void PulsoElemento(){
//Serial.print("PULSO EN ELEMENTO");
//Pregunta: quien determina la duración del pulso
}
//--------------------------------------------------
...
//pregunta: las puertas son los módulos WK0200
...
//--------------------------------------------------
//NO QUEDA CLARO EL Nº DE BYTES PARA CODIFICAR TERMOSTATO Y CONSIGNA
